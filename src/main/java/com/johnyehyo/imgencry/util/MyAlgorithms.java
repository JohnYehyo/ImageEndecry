...
public class MyAlgorithms {
    ...
    double rowEncrypt(int pixel[][], double x1, int i, int M, int N) {
        ...
        ArrayUtil.arr_copy(logistic_array, temp_logArr, N);
    ...
    //行置乱算法
    public void allRowEncrypt(int pixel[][], double x1, int M, int N) {
        //生成混沌序列
        double logistic_array[] = new double[N];
        produce_logisticArray(x1, logistic_array, N);
        //建立值与下标映射的Map
    ...
    void columnEncrypt_interface(int pixel[][], double x1, int M, int N) {
        ...
        //行列互换
        ArrayUtil.arr_change(pixel, temp, M, N);
        ...
        //再次行列互换
        ArrayUtil.arr_change(temp, temp2, N, M);
        //正式将原数组置乱
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < N; ++j) {
                pixel[i][j] = temp2[i][j];
    ...
    public void allRowDecrypt(int pixel[][], double x1, int M, int N) {
        //生成混沌序列
        double logistic_array[] = new double[N];
        produce_logisticArray(x1, logistic_array, N);
        //建立值与下标映射的Map
